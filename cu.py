...
root = Tk() # La ventana en si
root.title("Feet to Meters") # Titulo de la ventana
mainframe = ttk.Frame(root, padding="3 3 12 12") # La ventana de la ventana
...
ttk.Button(mainframe, text="CONVERT", command=lambda:convert(unitoutputbox.get(), input, unitinputbox.get())).grid(column=1, row=5, columnspan=2, padx=100, pady=(12, 0), sticky=(N, S, W, E))
...
quantityinput = ttk.Entry(mainframe, textvariable=input)
quantityinput.grid(column=1, row=2, sticky=(N, S, W, E))


# Bindings
root.bind('<Return>', lambda event:convert(unitoutputbox.get(), input, unitinputbox.get()))
# Configure the entry
input.set("")
quantityinput.focus()
